...
const KpiItem: React.FC<KpiItemProps> = ({ kpi, employeeId, period, config, kpiScore, isReadOnly = false }) => {
    ...
    const handleTargetSave = (value: string) => {
        updateKpiTarget(employeeId, kpi.id, value === '' ? null : parseFloat(value));
    };
            ...
            <div className="col-span-5 truncate font-medium flex items-center gap-1" title={config.name}>
                ...
                {targetIsEditable ? (
                    ...
                    kpi.target !== undefined && (
                         <span className="text-xs text-secondary">(هدف: {kpi.target.toLocaleString('fa-IR')})</span>
                    )
                )}
            </div>
            <div className="col-span-4">
                 <InlineEdit
                    value={kpi.scores[period] ?? null}
...
const KpiTabContent: React.FC<KpiTabContentProps> = ({ employee, period, finalScore, kpiConfigs, isReadOnly = false }) => {
    ...
    // Initialize the new KPI type dropdown
    useEffect(() => {
        if (!newKpiType && Object.keys(kpiConfigs).length > 0) {
            setNewKpiType(Object.keys(kpiConfigs)[0]);
        }
    }, [kpiConfigs, newKpiType]);
    ...
    const handleAddKpi = () => {
        if (!newKpiType) {
            showNotification('لطفاً نوع KPI را انتخاب کنید.', 'error');
            return;
        }
        // Prevent adding a duplicate KPI type
        if (employee.kpis.some(k => k.type === newKpiType)) {
            showNotification('این نوع KPI از قبل برای این کارمند وجود دارد.', 'info');
            return;
        }

        const targetValue = newKpiTarget ? parseFloat(newKpiTarget) : undefined;
        // Check if target is a valid number if provided
        if (newKpiTarget && isNaN(targetValue as number)) {
             showNotification('مقدار هدف نامعتبر است.', 'error');
            return;
        }

        addKpiToEmployee(employee.id, newKpiType, targetValue);
        showNotification('KPI با موفقیت افزوده شد.', 'success');
        ...
        <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-grow">
                ...
                {!isReadOnly && (
                    <>
                        <div className="border-t mt-3 pt-3" style={{borderColor: 'var(--border-color)'}}>
                            <h4 className="text-sm font-semibold mb-2">افزودن KPI جدید</h4>
                            <div className="flex flex-col sm:flex-row gap-2 items-center">
                                <select
                                    value={newKpiType}
                                    onChange={(e) => setNewKpiType(e.target.value)}
                                    className="w-full p-2 border rounded-lg bg-gray-50 text-gray-700 text-sm"
                                >
                                ...
                                <input
                                    ...
                                    placeholder="مقدار هدف (اختیاری)"
                                    className="w-full sm:w-48 p-2 border rounded-lg text-center bg-gray-50 text-gray-700 text-sm"
                                />
                                <button
                                    onClick={handleAddKpi}
                                    className="w-full sm:w-auto btn-primary text-white px-4 py-2 rounded-lg transition text-sm flex-shrink-0"
                                >
                                    افزودن
                                </button>
                            </div>
                ...
                {isReadOnly && note && (
                    <div className="border-t mt-3 pt-3" style={{borderColor: 'var(--border-color)'}}>
                        <label className="text-sm font-semibold">یادداشت برای دوره {period}</label>
                        <p className="w-full p-2 mt-2 rounded-lg text-sm min-h-[60px]" style={{backgroundColor: 'var(--bg-color)'}}>{note}</p>
                    </div>
                )}
            </div>
        </div>
